...
function ajax_posts_load(newQuery, extract)
{
  ...
  return new Promise(function (done) {
    ...
    $.get(url + '&' + $.param(query)).then(function (html) {
      ...
      var load_more = isEqual(old, query) && $parent.find('.ajax-posts .ajax-load-more-container').length > 0;
      var load_more_query = Object.assign({}, query);


      if($(html).is('.ajax-posts'))
      {
        html = $(html)[0].innerHTML;
      }

      if($(html).find('.ajax-posts').length)
      {
        html = $(html).find('.ajax-posts')[0].innerHTML;
      }
...
}).on('change', '.ajax-posts-load select', function () {

  var $this = $(this).children('option:selected').data('query');

  ajax_posts_load.call(this, $this, true);
});
