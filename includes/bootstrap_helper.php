  ...
  public static function picture($image, array $image_sizes = array(), $options = array())
  {
    ...
    <picture <?php echo $attributes ?>>
      <?php foreach($image_sizes as $breakpoint => $image_size): ?>
        ...
        <source media="(max-width: <?php echo esc_attr(static::$grid_max_breakpoints[$breakpoint]) ?>px)" srcset="<?php echo esc_attr($image['sizes'][$image_size]) ?>">
      <?php endforeach; ?>

      <img src="<?php echo esc_attr($image['sizes'][$largest]) ?>" alt="<?php echo esc_attr($image['name']) ?>">
    </picture>
    <?php
    return ob_get_clean();
  }
